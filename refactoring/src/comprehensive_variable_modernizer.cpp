...
public:
    ComprehensiveVariableModernizer() {
    ...
    void initializeTransformations() {
        ...
        transformations.push_back({
            std::regex(R"(^(\s*)const\s+auto\s+([a-zA-Z_][a-zA-Z0-9_]*)\s*=\s*([^;]+);$)"), 
            "$1const auto $2{$3};", 
            "const auto variable",
            false
        });
        ...
        transformations.push_back({
            std::regex(R"(^(\s*)auto\s+([a-zA-Z_][a-zA-Z0-9_]*)\s*=\s*([^;]+);$)"), 
            "$1auto $2{$3};", 
            "auto variable",
            false
        });
    ...
    bool isComplexExpression(const std::string& expression) {
        ...
        return expression.find("new ") != std::string::npos ||
               expression.find("static_cast") != std::string::npos ||
