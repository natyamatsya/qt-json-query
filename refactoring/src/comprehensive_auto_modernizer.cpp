...
public:
    ...
    void initializeTransformations() {
        ...
        transformations.push_back({
            std::regex(R"(^(\s*)static\s+const\s+((?:std::)?(?:string|vector|array|map|set|unordered_map|unordered_set|pair|tuple|shared_ptr|unique_ptr|optional|QJsonObject|QJsonArray|QJsonValue|QString|QStringView|QByteArray|QVariant|qsizetype|size_t|int|double|float|bool|uint32_t|uint64_t|int32_t|int64_t)(?:<[^>]*>)?(?:\s*\*)?)\s+([a-zA-Z_][a-zA-Z0-9_]*)\s*=\s*([^;]+);$)"), 
        ...
        transformations.push_back({
            std::regex(R"(^(\s*)const\s+((?:std::)?(?:string|vector|array|map|set|unordered_map|unordered_set|pair|tuple|shared_ptr|unique_ptr|optional|QJsonObject|QJsonArray|QJsonValue|QString|QStringView|QByteArray|QVariant|qsizetype|size_t|int|double|float|bool|uint32_t|uint64_t|int32_t|int64_t)(?:<[^>]*>)?(?:\s*\*)?)\s+([a-zA-Z_][a-zA-Z0-9_]*)\s*=\s*([^;]+);$)"), 
            "$1const auto $3 = $4;", 
            "const explicit type",
            true
        });
        ...
        transformations.push_back({
            std::regex(R"(^(\s*)((?:std::)?(?:string|vector|array|map|set|unordered_map|unordered_set|pair|tuple|shared_ptr|unique_ptr|optional|QJsonObject|QJsonArray|QJsonValue|QString|QStringView|QByteArray|QVariant|qsizetype|size_t|int|double|float|bool|uint32_t|uint64_t|int32_t|int64_t)(?:<[^>]*>)?(?:\s*\*)?)\s+([a-zA-Z_][a-zA-Z0-9_]*)\s*=\s*([^;]+);$)"), 
            "$1auto $3 = $4;", 
            "explicit type",
            true
        });
        ...
        transformations.push_back({
            std::regex(R"(^(\s*)([a-zA-Z_][a-zA-Z0-9_:]*::(?:const_)?iterator)\s+([a-zA-Z_][a-zA-Z0-9_]*)\s*=\s*([^;]+);$)"), 
            "$1auto $3 = $4;", 
            "iterator type",
            true
        });
        ...
        transformations.push_back({
            std::regex(R"(^(\s*)([a-zA-Z_][a-zA-Z0-9_:]*\*)\s+([a-zA-Z_][a-zA-Z0-9_]*)\s*=\s*((?:static_cast|reinterpret_cast|const_cast|dynamic_cast)<[^>]+>\([^)]+\));$)"), 
            "$1auto $3 = $4;", 
        ...
        transformations.push_back({
            std::regex(R"(^(\s*)((?:std::)?(?:shared_ptr|unique_ptr|weak_ptr)<[^>]+>)\s+([a-zA-Z_][a-zA-Z0-9_]*)\s*=\s*([^;]+);$)"), 
            "$1auto $3 = $4;", 
            "smart pointer type",
            true
        });
        ...
        transformations.push_back({
            std::regex(R"(^(\s*)(Q(?:JsonObject|JsonArray|JsonValue|String|StringView|ByteArray|Variant))\s+([a-zA-Z_][a-zA-Z0-9_]*)\s*=\s*([^;]+);$)"), 
            "$1auto $3 = $4;", 
            "Qt type",
            true
        });
        ...
        transformations.push_back({
            std::regex(R"(^(\s*)(std::(?:vector|array|map|set|unordered_map|unordered_set|list|deque|forward_list|stack|queue|priority_queue)<[^>]+>)\s+([a-zA-Z_][a-zA-Z0-9_]*)\s*=\s*([^;]+);$)"), 
            "$1auto $3 = $4;", 
            "STL container type",
            true
        });
        ...
        transformations.push_back({
            std::regex(R"(^(\s*)((?:int|double|float|bool|size_t|qsizetype|uint32_t|uint64_t|int32_t|int64_t))\s+([a-zA-Z_][a-zA-Z0-9_]*)\s*=\s*(static_cast<[^>]+>\([^)]+\));$)"), 
            "$1auto $3 = $4;", 
            ...
            for (const auto& transform : transformations) {
                if (transform.isSafe && std::regex_match(line, transform.pattern)) {
                    ...
                    if (newLine != line) {
                        ...
                        changesApplied++;
                        transformed = true;
                        std::cout << "  " << transform.description << ": " << originalLine.substr(0, 60) << "..." << std::endl;
                        break;
                    }
                }
            }
