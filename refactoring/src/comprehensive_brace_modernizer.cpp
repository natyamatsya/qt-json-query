...
private:
    ...
    void initializePatterns() {
        ...
        transformationPatterns.emplace_back(
            R"(^(\s*)(std::string\s+)([A-Za-z_][A-Za-z0-9_]*)\s*=\s*([^=][^;]*);)"
        );
        replacementPatterns.emplace_back("$1$2$3{$4};");
        ...
        transformationPatterns.emplace_back(
            R"(^(\s*)(QString\s+)([A-Za-z_][A-Za-z0-9_]*)\s*=\s*([^=][^;]*);)"
        );
        replacementPatterns.emplace_back("$1$2$3{$4};");
