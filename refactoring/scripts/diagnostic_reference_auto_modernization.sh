...
# Diagnostic Reference Auto Modernization Script
# Systematically applies reference auto modernization to all C++ files
# Validates build after each batch to ensure stability

set -e

PROJECT_ROOT="/Users/fischi/dev/github/natyamatsya/qt-json-query"
MODERNIZER="$PROJECT_ROOT/scripts/reference_auto_modernizer"
BUILD_DIR="$PROJECT_ROOT/build-relwithdebinfo-apple-clang"

cd "$PROJECT_ROOT"

echo "=== Diagnostic Reference Auto Modernization ==="
...
echo
echo "=== Starting Reference Auto Modernization ==="
...
# Batch 4: Core source files (JSONPath evaluation)
core_eval_files=(
...
# Batch 5: Core source files (JSONPath parsing and filtering)
core_parse_files=(
...
# Batch 6: Remaining core source files
...
# Batch 7: Header files (most critical)
header_files=(
...
if build_and_test; then
    echo "🎉 Reference Auto Modernization completed successfully!"
    echo "All batches processed and validated."
    echo
    echo "Summary:"
    echo "- All C++ files processed for reference auto modernization"
    echo "- Build stability maintained throughout"
    echo "- RFC 9535 compliance preserved"
    echo "- Reference semantics modernized to use auto&/const auto&"
else
