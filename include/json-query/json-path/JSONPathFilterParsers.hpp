...
parseSelfValue(QString s, std::vector<json_query::json_path::FilterFn>& out)
{
    if (const auto match = ctre::match<PAT>(to_sv(s)))
    {
        ...
        return parseRhsValue(op, value).and_then(
            [&](const ComparisonContext& ctx) -> std::optional<json_query::json_path::Token>
            {
                Builder b{out};
...
template <auto PAT>
[[nodiscard]] std::optional<json_query::json_path::Token>
parseRegex1(QString s, std::vector<json_query::json_path::FilterFn>& out)
{
    if (const auto match = ctre::match<PAT>(to_sv(s)))
    {
        const auto prop    = to_qstr(match.template get<1>().to_view());
...
std::optional<Token> parseEmbeddedCompareIndex(const QString& s)
{
    if (auto m = ctre::match<Pattern>(to_sv(s)))
    {
        ...
        // Embed filter directly into token
        token.embedFilter(
            [index, ctx = *ctx](const QJsonValue& j)
            {
                if (!j.isArray())
                    return false;
                const auto arr{j.toArray()};
