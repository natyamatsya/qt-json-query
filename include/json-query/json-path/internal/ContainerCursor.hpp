...
class alignas(32) ContainerCursor
{
  ...
  public:
    ...
    // ── Factory methods ─────────────────────────────────────────────────
    [[nodiscard]] static constexpr ContainerCursor object(const QJsonObject& o) noexcept
    {
        return {reinterpret_cast<std::uintptr_t>(&o), o.size()};
    }

    [[nodiscard]] static constexpr ContainerCursor array(const QJsonArray& a) noexcept
    {
    ...
    [[deprecated("Use range-based for loop or iterators instead")]] [[nodiscard]] bool hasNext() const noexcept
    {
        return m_idx < m_size;
    }

    [[deprecated("Use range-based for loop or iterators instead")]] [[nodiscard]] QJsonValue next() const noexcept
    {
    ...
    class iterator
    {
      public:
        ...
        {
            if (auto cmp = m_tagged <=> other.m_tagged; cmp != 0)
                return cmp;
        ...
        [[nodiscard]] constexpr bool operator==(std::default_sentinel_t) const noexcept { return m_idx >= m_size; }
#endif
  ...
  private:
    ...
    constexpr ContainerCursor(std::uintptr_t tagged, qsizetype size) noexcept : m_tagged(tagged), m_size(size) {}
...
constexpr ContainerCursor::iterator ContainerCursor::begin() const noexcept { return iterator(m_tagged, 0, m_size); }
...
constexpr auto begin(const ContainerCursor& cursor) noexcept -> ContainerCursor::iterator { return cursor.begin(); }

constexpr auto end(const ContainerCursor& cursor) noexcept -> ContainerCursor::iterator { return cursor.end(); }
