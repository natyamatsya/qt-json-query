...
class PathPatternDetector
{
  public:
    ...
    QT_QUERY_JSON_ALWAYS_INLINE static PathPattern detectPattern(const std::vector<Token>& tokens) noexcept
    {
        ...
        { // Reduced from 4 to 3 for safety
  ...
  private:
    ...
    QT_QUERY_JSON_ALWAYS_INLINE static bool isSimpleKey(const QString& key) noexcept
    {
        ...
        if (key.length() > 64)
        { // Reasonable limit for simple keys
            return false;
        }
...
class PatternAwarePathEvaluator
{
  public:
     ...
     */
    static std::expected<QJsonArray, EvalError>
    evaluate(const detail::PathEvalCtx& ctx, const std::vector<Token>& tokens, const QJsonValue& root) noexcept
    {
