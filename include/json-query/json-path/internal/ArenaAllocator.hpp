...
class ArenaAllocator
{
  public:
    ...
    Stats getStats() const
    {
        ...
        double utilization =
            totalAllocated_ > 0 ? (static_cast<double>(totalAllocated_) / (blocks_.size() * blockSize_) * 100.0) : 0.0;

        return {totalAllocated_, blocks_.size(), currentBlockUsed, currentBlockRemaining, utilization};
    }
...
class ArenaSTLAllocator
{
  public:
    ...
    pointer allocate(size_type n) { return reinterpret_cast<pointer>(arena_->allocate(n * sizeof(T), alignof(T))); }
    ...
    bool operator==(const ArenaSTLAllocator& other) const { return arena_ == other.arena_; }

    bool operator!=(const ArenaSTLAllocator& other) const { return !(*this == other); }
...
class ThreadLocalArena
{
  public:
    ...
    static ArenaAllocator::Stats getStats() { return get().getStats(); }
};
