...
public:
     ...
     */
    static constexpr bool requiresContext(Token::Kind kind) {
        return getMetadata(kind).requiresContext;
    }
    
    /**
     * @brief Check if token can multiply results
     */
    static constexpr bool canMultiply(Token::Kind kind) {
        return getMetadata(kind).canMultiply;
    }
    
    /**
     * @brief Get token name for debugging
     */
    static constexpr const char* getName(Token::Kind kind) {
        return getMetadata(kind).name;
    }
    
    /**
     * @brief Get token priority for optimization
     */
    static constexpr int getPriority(Token::Kind kind) {
        return getMetadata(kind).priority;
    }
};
