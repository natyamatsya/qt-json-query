...
public:
    ...
    CacheOptimizedStackFrame& top() {
        return *frames_.back();
    }
    
    const CacheOptimizedStackFrame& top() const {
        return *frames_.back();
    }
...
public:
    explicit CacheOptimizedResultCollector(size_t estimatedSize = 64) 
    ...
    const QJsonArray& getResults() const {
        return results_;
    }
    ...
    const QString& intern(QStringView key) {
        ...
        if (cache_.size() >= MAX_CACHE_SIZE) {
            ...
            auto minIt = std::min_element(cache_.begin(), cache_.end(),
                [](const CachedKey& a, const CachedKey& b) {
