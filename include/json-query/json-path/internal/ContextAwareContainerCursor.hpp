...
public:
    ...
    [[nodiscard]] constexpr const QJsonValue& rootDocument() const noexcept {
        return ctx_->rootDocument;
    }
    ...
    [[nodiscard]] constexpr const detail::PathEvalCtx& evalContext() const noexcept {
        return *ctx_;
    }
};
...
public:
    ...
    [[nodiscard]] static constexpr ContextAwareContainerCursor object(
        const QJsonObject& obj, Provider provider) noexcept {
        return {ContainerCursor::object(obj), std::move(provider)};
    }
    
    [[nodiscard]] static constexpr ContextAwareContainerCursor array(
        const QJsonArray& arr, Provider provider) noexcept {
        return {ContainerCursor::array(arr), std::move(provider)};
    }
    
    // STL-compatible container interface
    [[nodiscard]] constexpr iterator begin() const noexcept {
        return iterator{cursor_.begin(), &provider_};
    }
    
    [[nodiscard]] constexpr iterator end() const noexcept {
        return iterator{cursor_.end(), &provider_};
    }
    ...
    [[nodiscard]] constexpr const ContainerCursor& cursor() const noexcept {
        return cursor_;
    }
    
    [[nodiscard]] constexpr const Provider& contextProvider() const noexcept {
        return provider_;
    }
 ...
 */
[[nodiscard]] constexpr PathEvalContextCursor makeContextAwareCursor(
    const QJsonObject& obj, const detail::PathEvalCtx& ctx, const QJsonValue& current) noexcept {
    return PathEvalContextCursor::object(obj, PathEvalContextProvider{ctx, current});
}

[[nodiscard]] constexpr PathEvalContextCursor makeContextAwareCursor(
    const QJsonArray& arr, const detail::PathEvalCtx& ctx, const QJsonValue& current) noexcept {
 ...
 */
[[nodiscard]] constexpr SimpleContextCursor makeSimpleContextCursor(
    const QJsonObject& obj, const QJsonValue& root, const QJsonValue& current) noexcept {
    return SimpleContextCursor::object(obj, SimpleContextProvider{root, current});
}

[[nodiscard]] constexpr SimpleContextCursor makeSimpleContextCursor(
    const QJsonArray& arr, const QJsonValue& root, const QJsonValue& current) noexcept {
 ...
 */
template<typename RootFunc, typename CurrentFunc>
[[nodiscard]] constexpr auto makeLambdaContextCursor(
    const QJsonObject& obj, RootFunc&& rootFn, CurrentFunc&& currentFn) noexcept {
...
template<typename RootFunc, typename CurrentFunc>
[[nodiscard]] constexpr auto makeLambdaContextCursor(
    const QJsonArray& arr, RootFunc&& rootFn, CurrentFunc&& currentFn) noexcept {
    using Provider = LambdaContextProvider<std::decay_t<RootFunc>, std::decay_t<CurrentFunc>>;
 ...
 */
template<ContextProvider Provider>
[[nodiscard]] constexpr ContextAwareContainerCursor<Provider> makeContextCursor(
    const QJsonObject& obj, Provider provider) noexcept {
