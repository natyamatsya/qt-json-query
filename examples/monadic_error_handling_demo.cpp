...
std::expected<JSONPath, QString> createOptimizedPath(const QString& validPath)
{
    ...
    if (!result)
        return std::unexpected(QStringLiteral("JSONPath creation failed: ") + result.error().formatted_message());
...
std::expected<QJsonArray, QString> processJsonPathMonadic(const QString& jsonStr, const QString& pathStr)
{
    ...
    return validateJsonDocument(jsonStr)
        .and_then(
            [](const QJsonDocument& doc) -> std::expected<QJsonDocument, QString>
            {
                std::cout << "  ✅ JSON document validated\n";
                return doc;
            })
        .and_then(
            [&pathStr](const QJsonDocument& doc) -> std::expected<std::pair<QJsonDocument, QString>, QString>
            {
                return validatePathSyntax(pathStr).transform(
                    [&doc](const QString& validPath)
                    {
        ...
        .and_then(
            [](const std::pair<QJsonDocument, QString>& docAndPath)
                -> std::expected<std::pair<QJsonDocument, JSONPath>, QString>
            {
                const auto& [doc, path] = docAndPath;
                return createOptimizedPath(path).transform(
        ...
        .and_then(
            [](const std::pair<QJsonDocument, JSONPath>& docAndPath) -> std::expected<QJsonArray, QString>
            {
                const auto& [doc, path] = docAndPath;
...
std::expected<QJsonArray, QString> processJsonPathTraditional(const QString& jsonStr, const QString& pathStr)
{
...
int main()
{
    ...
    std::vector<TestCase> testCases = {{validJson, "$.store.book[*].title", "Valid: Extract all book titles"},
