...
TEST_F(ContextAwareContainerCursorTest, ConceptRequirements)
{
    ...
    QJsonValue dummy;
    auto       lambdaProvider = LambdaContextProvider{[&dummy]() -> const QJsonValue& { return dummy; },
...
TEST_F(ContextAwareContainerCursorTest, STLAlgorithmCompatibility)
{
    ...
    // Test std::count_if
    auto activeCount = std::count_if(cursor.begin(),
                                     cursor.end(),
                                     [](const auto& item)
                                     {
    ...
    // Test std::find_if
    auto foundIt = std::find_if(cursor.begin(),
                                cursor.end(),
                                [](const auto& item)
                                {
                                    const auto& [value, context] = item;
    ...
    auto allHaveRootName = std::all_of(cursor.begin(),
                                       cursor.end(),
                                       [](const auto& item)
                                       {
                                           const auto& [value, context] = item;
...
TEST_F(ContextAwareContainerCursorTest, JSONPathIntegrationScenario)
{
    ...
    complexRoot["users"]  = QJsonArray{QJsonObject{{"id", 1}, {"name", "Alice"}, {"role", "admin"}},
