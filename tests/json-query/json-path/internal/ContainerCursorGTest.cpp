...
TEST_F(ContainerCursorTest, STLAlgorithms)
{
    ...
    // Test std::count_if
    auto stringCount = std::count_if(cursor.begin(), cursor.end(), [](const QJsonValue& v) { return v.isString(); });
    EXPECT_EQ(stringCount, 1);

    // Test std::find_if
    auto foundIt = std::find_if(
        cursor.begin(), cursor.end(), [](const QJsonValue& v) { return v.isDouble() && v.toInt() == 123; });
    EXPECT_NE(foundIt, cursor.end());
    EXPECT_EQ((*foundIt).toInt(), 123);

    // Test std::all_of
    auto allValid = std::all_of(cursor.begin(), cursor.end(), [](const QJsonValue& v) { return !v.isUndefined(); });
...
TEST_F(ContainerCursorTest, STLAccumulate)
{
    ...
    // Test std::accumulate
    auto sum = std::accumulate(cursor.begin(),
                               cursor.end(),
                               0,
                               [](int acc, const QJsonValue& v) { return acc + (v.isDouble() ? v.toInt() : 0); });
...
TEST_F(ContainerCursorTest, RangesSupport)
{
    ...
    auto stringCount = std::ranges::count_if(cursor, [](const QJsonValue& v) { return v.isString(); });
    EXPECT_EQ(stringCount, 1);

    // Test ranges views (if available)
    auto validValues = cursor | std::views::filter([](const QJsonValue& v) { return !v.isUndefined(); });
...
TEST_F(ContainerCursorTest, EmptyContainerIteration)
{
    ...
    auto emptyCount = std::count_if(objCursor.begin(), objCursor.end(), [](const QJsonValue&) { return true; });
...
TEST_F(ContainerCursorTest, ObjectValueAccess)
{
    ...
    {
        if (val.isString() && val.toString() == "test_object")
            hasString = true;
        if (val.isDouble() && val.toInt() == 42)
            hasInt = true;
        if (val.isBool() && val.toBool() == true)
            hasBool = true;
        if (val.isDouble() && val.toDouble() == 98.5)
            hasDouble = true;
        if (val.isObject())
            hasObject = true;
    }
...
TEST_F(ContainerCursorTest, ComplexNestedStructure)
{
    QJsonObject complex;
    complex["users"] =
        QJsonArray{QJsonObject{{"id", 1}, {"name", "Alice"}, {"roles", QJsonArray{"admin", "user"}}},
    ...
    auto usersIt = std::find_if(cursor.begin(), cursor.end(), [](const QJsonValue& v) { return v.isArray(); });
...
TEST_F(ContainerCursorTest, PerformanceBenchmark)
{
    ...
    for (int i = 0; i < size; ++i)
        veryLargeArray.append(QJsonObject{{"id", i}, {"value", i * 2}, {"name", QString("item_%1").arg(i)}});
