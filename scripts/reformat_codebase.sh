...
# Comprehensive Code Reformatting Script
...
echo "🎨 Starting comprehensive code reformatting with .clang-format"
...
echo "📋 Preview of files to format:"
echo "$FILES_TO_FORMAT" | head -10
...
read -p "🤔 Proceed with reformatting $FILE_COUNT files? [y/N]: " -n 1 -r
echo
if [[ ! $REPLY =~ ^[Yy]$ ]]; then
    echo "❌ Reformatting cancelled"
    exit 0
fi
