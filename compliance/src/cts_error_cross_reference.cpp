...
int main() {
    ...
    std::vector<TestCase> testCases = {
        ...
        TestCase{"name shorthand, absent data", "$.c", 
         false, QJsonObject{{"a", "A"}, {"b", "B"}}, QJsonArray()},
        
        // Index out of bounds scenarios
        TestCase{"out of bound", "$[2]", 
         false, QJsonArray{"first", "second"}, QJsonArray()},
        TestCase{"negative out of bound", "$[-3]", 
         false, QJsonArray{"first", "second"}, QJsonArray()},
         
        // Type mismatch scenarios
        TestCase{"index on object", "$[0]", 
         false, QJsonObject{{"key", "value"}}, QJsonArray()},
        TestCase{"key on array", "$.key", 
         false, QJsonArray{"first", "second"}, QJsonArray()},
         
        // Slice scenarios
        TestCase{"slice on object", "$[0:2]", 
         false, QJsonObject{{"key", "value"}}, QJsonArray()},
        TestCase{"zero step slice", "$[0:2:0]", 
         false, QJsonArray{"a", "b", "c"}, QJsonArray()},
    };
