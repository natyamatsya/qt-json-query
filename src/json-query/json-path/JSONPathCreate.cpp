...
{
    qDebug() << "DEBUG: JSONPath::create() called with rawPath=" << rawPath;
    qCDebug(jsonPathLog) << "JSONPath::create() called with rawPath=" << rawPath;
    auto compileResult = compile(rawPath);

    if (!compileResult)
    {
        qDebug() << "DEBUG: JSONPath::create() compile FAILED with error:" << to_qt_sv(compileResult.error());
    ...
    qDebug() << "DEBUG: JSONPath::create() compile SUCCEEDED, creating JSONPath with" << compileResult->compiled.tokens.size() << "tokens";
    for (size_t i = 0; i < compileResult->compiled.tokens.size(); ++i) {
        qDebug() << "DEBUG: Token[" << i << "] kind=" << static_cast<int>(compileResult->compiled.tokens[i].kind) 
