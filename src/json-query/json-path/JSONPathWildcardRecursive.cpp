...
struct RecursivePatternDetector
{
    ...
    static QStringView detectEarlyTerminationPattern(QStringView pathExpression)
    {
        ...
        if (pathExpression.startsWith(u"$..") && pathExpression.size() > 3)
        {
            ...
            if (!fieldName.contains(u'[') && !fieldName.contains(u'.') && !fieldName.contains(u'?') &&
                !fieldName.contains(u'*'))
            {
 ...
 */
static std::expected<QJsonArray, EvalError> evaluateRecursiveDirectFastPath(const QJsonValue& root,
...
static std::expected<QJsonArray, EvalError> evaluateRecursiveOptimized(const QJsonValue& value,
                                                                       QStringView       pathHint = QStringView())
{
    ...
    if (!targetField.isEmpty() && complexity > 200)
    {
        ...
        auto result = IterativeRecursiveDescent::evaluateIterativePhase3Optimized(value, targetField, collector);
    ...
    if (!targetField.isEmpty() && complexity > 25)
    {
        ...
        auto result = IterativeRecursiveDescent::evaluateIterativeWithEarlyTermination(value, targetField, collector);
...
std::expected<QJsonArray, EvalError> wildcardObject(const QJsonObject& obj) { return evaluateWildcardObjectImpl(obj); }
