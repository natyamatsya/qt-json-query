...
std::optional<Slice> makeSlice(QStringView v)
{
    ...
    if (parts.size() > 0 && !strictParse(parts[0].trimmed(), startOpt, /*overflowInvalid=*/true))
    {
        qCDebug(jsonPathLog) << "strictParse failed for start";
        return std::nullopt;
    }
    if (parts.size() > 1 && !strictParse(parts[1].trimmed(), endOpt, /*overflowInvalid=*/true))
    {
        qCDebug(jsonPathLog) << "strictParse failed for end";
        return std::nullopt;
    }
    if (parts.size() > 2 && !strictParse(parts[2].trimmed(), stepOpt, /*overflowInvalid=*/true))
    {
        qCDebug(jsonPathLog) << "strictParse failed for step";
        return std::nullopt;
    }
    ...
    auto start = startOpt.has_value() ? *startOpt : SENTINEL;
    auto end   = endOpt.has_value() ? *endOpt : SENTINEL;
...
bool isValidQuotedKey(QStringView key, QuoteStyle style)
{
    ...
    for (qsizetype i = 0; i < key.size(); ++i)
    {
        ...
        if (!escOk)
        {
            if (style == QuoteStyle::Double && esc == u'\"')
                escOk = true;
            else if (style == QuoteStyle::Single && esc == u'\'')
                escOk = true;
        }
        if (!escOk)
        ...
        if (esc == u'u')
        {
            ...
            for (int k = 1; k <= 4; ++k)
            {
                ...
                if (h.isDigit())
                    val = h.digitValue();
                else if (h.toLower() >= u'a' && h.toLower() <= u'f')
                    val = 10 + (h.toLower().unicode() - u'a');
                if (val < 0)
                    return false;
            ...
            else if (isHighSurrogate)
            {
                ...
                if (i + 11 >= key.size())
                    return false; // need another 6 chars: \uXXXX
                if (key[i + 6] != u'\\' || key[i + 7] != u'u')
                    return false; // not another \u
