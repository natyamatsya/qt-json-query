...
class MemoryOptimizationTest
{
  ...
  private:
    ...
    static TestResult
    benchmarkOptimization(const std::string& testName, const QString& jsonPath, const QJsonValue& testData)
    {
        try
        {
            // Compile JSONPath
...
int main()
{
    ...
    {
        std::cout << "| " << result.testName << " | " << static_cast<long>(result.baselineDurationNs) << " | "
                  << static_cast<long>(result.optimizedDurationNs) << " | " << std::fixed << std::setprecision(1)
                  << result.improvementPercent << "%"
                  << " | " << result.allocationReduction << " | " << (result.rfcCompliant ? "Yes" : "No") << " |\n";

        totalImprovement += result.improvementPercent;
        totalAllocReduction += result.allocationReduction;
        if (result.rfcCompliant)
            compliantTests++;
    }
