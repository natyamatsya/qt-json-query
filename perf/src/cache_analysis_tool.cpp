...
struct CacheMetrics
{
    ...
    void calculateCacheMetrics()
    {
        ...
        for (size_t i = 1; i < accessedAddresses.size(); ++i)
        {
            ...
            if (distance > CACHE_LINE_SIZE * 16)
            { // Beyond L1 cache reach
                cacheMisses++;
            }
        }
...
class CacheAnalyzer
{
  ...
  public:
    CacheAnalyzer(const std::string& profilePrefix = "cache_analysis") : profilePrefix_(profilePrefix)
    {
#ifdef HAVE_GPERFTOOLS
...
class TestDataGenerator
{
  public:
    ...
    {
        QJsonObject root;
        root["store"] = QJsonObject{{"book", generateLargeArray(100)},
  ...
  private:
    ...
    void analyzeRecursivePattern(const QJsonValue& data, const QString& pattern, const QString& testName)
    {
        ...
        std::cout << "Cache miss ratio: " << std::fixed << std::setprecision(2)
                  << (metrics.memoryAccesses > 0 ? (double)metrics.estimatedCacheMisses / metrics.memoryAccesses * 100
                                                 : 0)
                  << "%" << std::endl;
        std::cout << "Avg access distance: " << std::fixed << std::setprecision(1) << metrics.avgAccessDistance
                  << " bytes" << std::endl;
