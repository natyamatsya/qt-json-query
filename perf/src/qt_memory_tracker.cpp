...
class QtMemoryTracker
{
  public:
    ...
    static OperationStats
    benchmarkOperation(const QString& jsonPath, const QJsonValue& testData, int iterations = 1000)
    {
        // Compile JSONPath
...
class StreamingComparisonTest
{
  public:
    static void runComparison()
    {
        ...
        for (int i = 0; i < 100; ++i)
        { // Larger dataset for better memory analysis
            QJsonObject item;
            item["id"]    = i;
        ...
        for (const auto& [testName, jsonPath] : testCases)
        {
            try
            {
                ...
                double memoryEfficiency =
                    stats.resultArraySize > 0 ? (avgDuration / stats.resultArraySize) : avgDuration;

                std::cout << "| " << testName.toStdString() << " | " << static_cast<long>(avgDuration) << " | "
                          << stats.resultArraySize << " | " << stats.peakArraySize << " | " << std::fixed
                          << std::setprecision(2) << memoryEfficiency << " |\n";
            }
