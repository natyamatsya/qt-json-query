...
class AllocationHotspotAnalyzer
{
  ...
  private:
    ...
    static HotspotResult analyzeOperation(const std::string& testName,
                                          const QString&     jsonPath,
                                          const QJsonValue&  testData,
                                          const std::string& description)
    {
        try
        {
            ...
            return {testName,
                    static_cast<size_t>(estimatedPeakMB),
                    avgDuration,
                    static_cast<size_t>(avgResults),
                    memoryPerResult,
                    bottleneck};
        }
...
int main()
{
    ...
    std::cout << "| Operation | Peak Memory (MB) | Avg Duration (ns) | Result Count | Memory/Result | Primary "
                 "Bottleneck |\n";
    std::cout << "|-----------|------------------|-------------------|--------------|---------------|-----------------"
                 "---|\n";
    ...
    {
        std::cout << "| " << result.operation << " | " << result.peakMemoryMB << " | "
                  << static_cast<long>(result.avgDurationNs) << " | " << result.resultCount << " | " << std::fixed
                  << std::setprecision(2) << result.memoryPerResult << " | " << result.bottleneck << " |\n";
    }
